...
class SNNModel17(SlideScene):
    def construct(self):
        note = "אם כך סי פלוס ודי פלוס הם קבוצת איבנטים שנורו משני המרחבים בזמן טי והחיתוך בינהם הוא קבוצת האיבנטים שנורו משתי הקבוצות באותו הזמן מגדירים את הקבוצה או פלוס"
        self.create_note(note)
        for x in Header().get():
            self.add(x)
        name = Text("Guy Tordjman", font_size=11).shift(3.2*DOWN+6.3*LEFT)
